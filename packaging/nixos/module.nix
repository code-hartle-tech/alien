...
in
{
  options.services.alien = {
    enable = lib.mkEnableOption "Alien — Acer Predator/Nitro hardware control";
    ...
    users = lib.mkOption {
      ...
      description = ''
        ...
        lighting and the turbo flags. This is a real capability — grant it as
        deliberately as any other hardware group.
    ...
    profileOnBoot = lib.mkOption {
      ...
      description = ''
        ...
        turbo flags off) measured roughly +48% sustained CPU throughput on a
        Helios 300 PH315-53. It is loud. That is the trade.
      '';
    };
  };
  ...
  config = lib.mkIf cfg.enable {
    ...
    systemd.services.alien-daemon = {
      ...
      serviceConfig = {
        ...
        # to group `alien` is the daemon's one privileged call, and without it
        # seccomp raises SIGSYS and the process core-dumps at startup rather
