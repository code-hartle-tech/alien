...
fn typed_feature(
    ...
    match request {
        ...
        FeatureRequest::SetLcdOverdrive(enabled) => guard
            ...
            .map(|state| state.map(|enabled| vec![enabled as u8]).unwrap_or_default()),
        FeatureRequest::GetBootAnimation => guard
            .boot_animation()
            .map(|state| state.map(|enabled| vec![enabled as u8]).unwrap_or_default()),
        FeatureRequest::SetBootAnimation(enabled) => guard
            .set_boot_animation(enabled)
            .map(|state| state.map(|enabled| vec![enabled as u8]).unwrap_or_default()),
...
impl FeatureRequest {
    ...
    fn is_mutation(self) -> bool {
        matches!(
            ...
            FeatureRequest::SetCoolBoost(_)
                ...
                | FeatureRequest::SetBootAnimation(_)
    ...
    fn audit_name(self) -> &'static str {
        match self {
            ...
            FeatureRequest::GetBootAnimation => "boot-animation get",
            FeatureRequest::SetBootAnimation(false) => "boot-animation off",
            FeatureRequest::SetBootAnimation(true) => "boot-animation on",
...
fn parse_feature_request(line: &str) -> Option<FeatureRequest> {
    ...
    Some(match parts.as_slice() {
        ...
        ["FEATURE", "boot-animation", "get"] => FeatureRequest::GetBootAnimation,
        ["FEATURE", "boot-animation", "set", "0"] => FeatureRequest::SetBootAnimation(false),
        ["FEATURE", "boot-animation", "set", "1"] => FeatureRequest::SetBootAnimation(true),
