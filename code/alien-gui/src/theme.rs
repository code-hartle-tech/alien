...
use eframe::egui::{self, Color32, CornerRadius, FontFamily, FontId, Pos2, Rect, Stroke, StrokeKind};
...
/// are subset to Latin-1 plus the punctuation this UI uses, so anything
/// outside that — a translated model name out of DMI, say — would otherwise
...
fn install_fonts(ctx: &egui::Context) {
    ...
    for (name, bytes) in [
        (SANS, &include_bytes!("../assets/fonts/Archivo-Regular.ttf")[..]),
        (SANS_B, &include_bytes!("../assets/fonts/Archivo-SemiBold.ttf")[..]),
        (MONO, &include_bytes!("../assets/fonts/IBMPlexMono-Regular.ttf")[..]),
        (MONO_B, &include_bytes!("../assets/fonts/IBMPlexMono-SemiBold.ttf")[..]),
    ] {
        f.font_data
...
pub fn tracked_width(ctx: &egui::Context, text: &str, font: &FontId, tracking: f32) -> f32 {
    ...
    let glyphs: f32 = text.chars().map(|c| ctx.fonts(|f| f.glyph_width(font, c))).sum();
...
pub fn vrule(ui: &mut egui::Ui, height: f32) {
    ...
    if ui.is_rect_visible(rect) {
        ui.painter().vline(rect.center().x, rect.y_range(), hair(LINE));
    }
}
...
pub fn chip(ui: &mut egui::Ui, label: &str, style: ChipStyle, size: f32) -> egui::Response {
    ...
    let border = if resp.hovered() && style != ChipStyle::Disabled { BRIGHT } else { border };
...
pub fn tag(
    ...
    let border = if resp.hovered() && enabled { BRIGHT } else { border };
    let p = ui.painter();
    p.rect_filled(rect, 0.0, fill);
    p.rect_stroke(rect, 0.0, hair(border), StrokeKind::Inside);
    let cx = rect.left() + (w - tw - mark_w) / 2.0;
    tracked(ui.ctx(), ui.painter(), Pos2::new(cx, rect.center().y), label, font, text, 0.5);
    if marked {
        crate::glyph::draw(
            ui.painter(),
...
pub fn section(ui: &mut egui::Ui, title: &str, meta: Option<&str>) {
    ...
    let y = rect.center().y;
    let mut x = rect.left();
    x += tracked(ui.ctx(), ui.painter(), Pos2::new(x, y), title, font, MUTED, 3.0) + 12.0;
...
pub fn slider(ui: &mut egui::Ui, value: &mut u8, range: std::ops::RangeInclusive<u8>, width: f32, enabled: bool) -> egui::Response {
    ...
    if enabled {
        p.rect_filled(Rect::from_min_size(Pos2::new(rect.left(), y - 2.0), egui::vec2(rect.width(), 4.0)), 0.0, LINE);
        p.rect_filled(Rect::from_min_size(Pos2::new(rect.left(), y - 2.0), egui::vec2(hx - rect.left(), 4.0)), 0.0, GREEN);
        ...
        while x < rect.right() {
            ...
            p.rect_filled(Rect::from_min_max(Pos2::new(x, y - 2.0), Pos2::new(end, y + 2.0)), 0.0, LINE);
            x += 10.0;
        }
...
pub fn badge(ui: &mut egui::Ui, mark: crate::glyph::Mark, label: &str, colour: Color32, dashed: bool) {
    ...
    crate::glyph::draw(p, Pos2::new(rect.left() + 12.0, rect.center().y), mark, 8.0, colour);
    p.text(
