...
use alien_core::attribution::{cpu_sysfs_root, ThrottleCounters};
use alien_core::gaming;
use alien_core::lease;
...
use alien_core::wmi::{FanMode, OverclockTarget};
use alien_core::{
    covini_brightness, covini_speed, Colour, Device, Direction, Effect, Fan, FanBehaviour, GpuMode,
    GpuModeOptIn, GpuModeState, Zone, GPU_MODE_ACKNOWLEDGEMENT,
};
...
COMMANDS
    ...
    fan <cpu|gpu> auto|max    one fan only; the other is left untouched
    limiters [watch <secs>]   which cap is binding: throttle event counters
    ...
    boot-animation status|on|off  POST animation; offered only if the getter supports it
    ...
    profile snapshot <name>   capture live hardware state as a profile

    gamesync begin <profile>  apply a profile with a crash-safe restore recorded
    gamesync end              restore the baseline and clear the lease
    gamesync recover          restore after a holder died without cleaning up
    gamesync status           show any outstanding lease

    gaming status             DLSS and shader-cache levers, and whether they are set
    gaming apply              write the session-wide environment.d drop-in
    gaming launch-options     the Steam launch line for per-game levers
...
NOTES
    ...
    worth +61.8% sustained CPU throughput, because the stock EC curve holds
    the chip in thermal throttle. Both conditions pin the CPU at 92 C; the
    difference is the clock underneath, 1446 MHz against 2406 MHz.
";
...
fn run(a: &[&str]) -> Result<(), String> {
    match a {
        ...
        // Which cap is binding right now. Every negative result this project
        // recorded came from tuning a limiter that was not the active one.
        ["limiters"] => {
            let counters = ThrottleCounters::read(&cpu_sysfs_root());
            println!("CPU throttle counters (monotonic since boot)");
            print_counter("  package thermal", counters.package_throttle);
            print_counter("  package power  ", counters.package_power_limit);
            print_counter("  core thermal   ", counters.core_throttle);
            println!(
                "\nA single reading proves nothing - counters only rise. Run\n\
                 `alien limiters watch <secs>` to count events across a workload."
            );
            Ok(())
        }
        ["limiters", "watch", secs] => {
            ...
            let root = cpu_sysfs_root();
            let before = ThrottleCounters::read(&root);
            println!("sampling for {secs}s...");
            std::thread::sleep(std::time::Duration::from_secs(secs));
            let delta = ThrottleCounters::read(&root).delta(&before);
            println!("throttle events in {secs}s");
            print_counter("  package thermal", delta.package_throttle);
            print_counter("  package power  ", delta.package_power_limit);
            print_counter("  core thermal   ", delta.core_throttle);
            println!(
                "\nverdict: {}",
                if delta.any_throttling() {
                    "THERMALLY LIMITED - cooling is the binding constraint"
                } else {
                    "no throttling observed in this window"
                }
            );
            Ok(())
        }
        ...
        // Per-fan auto, leaving the other fan alone. This is the mixed case
        // PredatorSense's Custom mode allows and Alien could not express: the
        // core has always encoded the single-fan word, but every frontend only
        // offered a global Auto.
        ["fan", which, "auto"] => {
            let fan = parse_fan(which)?;
            let d = open()?;
            d.set_fan_behaviour(FanBehaviour::Single {
                fan,
                mode: FanMode::Auto,
            })
            .map_err(fmt)?;
            println!("{which} fan handed to the EC curve; other fan unchanged");
            report_fans(&d);
            Ok(())
        }
        ["fan", which, "max"] => {
            ...
            .map_err(fmt)?;
            println!("{which} fan to maximum; other fan unchanged");
            report_fans(&d);
            Ok(())
        }
        ["fan", which, pct] => {
        ...
        ["boot-animation", "status"] => {
            match open()?.boot_animation().map_err(fmt)? {
                Some(enabled) => println!(
                    "Boot animation {} (firmware getter)",
                    if enabled { "on" } else { "off" }
                ),
                None => println!("Boot animation unsupported by the live firmware getter"),
            }
            Ok(())
        }
        ["boot-animation", value @ ("on" | "off")] => {
            let enabled = *value == "on";
            match open()?.set_boot_animation(enabled).map_err(fmt)? {
                Some(confirmed) => println!(
                    "Boot animation {} (getter-confirmed)",
                    if confirmed { "on" } else { "off" }
                ),
                None => return Err("Boot animation is not supported by this live getter".into()),
            }
            Ok(())
        }
        ...
        // Apply a profile for the duration of something else, and always get
        // back. The baseline is written to disk *before* the hardware is
        // touched, so a SIGKILL between the two leaves a recoverable machine —
        // which is the case gamemode loses, because its baseline is in RAM.
        // Software levers that actually move frames. No firmware involved —
        // these are environment variables the driver reads at process start.
        ["gaming", "status"] => {
            println!("shader cache and DLSS environment levers\n");
            for l in gaming::levers() {
                let scope = if l.global_only { "session-wide" } else { "per game" };
                match gaming::is_set(&l) {
                    Some(v) if v == l.value => println!("  [set]     {} = {v}", l.key),
                    Some(v) => println!("  [differs] {} = {v} (recommended {})", l.key, l.value),
                    None => println!("  [unset]   {} ({scope})", l.key),
                }
                println!("            {}", l.why);
            }
            println!(
                "\nNote: `alien gaming status` reads this process's environment, so\n\
                 session-wide settings show as unset until the next login."
            );
            Ok(())
        }
        ["gaming", "launch-options"] => {
            println!("{}", gaming::steam_launch_options());
            Ok(())
        }
        ["gaming", "apply"] => {
            ...
            let path = dir.join("50-alien-gaming.conf");
            std::fs::write(&path, gaming::environment_d_dropin()).map_err(|e| e.to_string())?;
            println!("wrote {}", path.display());
            println!("takes effect for sessions started after the next login");
            println!("\nPer-game, paste into Steam launch options:");
            println!("  {}", gaming::steam_launch_options());
            Ok(())
        }
        ...
        ["gamesync", "begin", name] => {
            ...
            let d = open()?;
            // Lighting is readable; fan mode is not, so the baseline records
            // only what the firmware can actually attest to.
            let baseline = Profile::snapshot(&d, "alien-gamesync-baseline".into(), None).ok();
            if baseline.is_none() {
                println!(
                    "warning: could not capture a baseline; recovery will fall back to fans-max"
                );
            }
            lease::acquire(&lease::Lease {
                holder: std::env::var("ALIEN_GAMESYNC_HOLDER").unwrap_or_else(|_| "gamemode".into()),
                applied: p.name.clone(),
                baseline,
            })
            .map_err(|e| e.to_string())?;
            p.apply(&d).map_err(fmt)?;
            println!("gamesync: applied {} (restore recorded)", p.name);
            Ok(())
        }
        ["gamesync", "end"] => {
            let Some(l) = lease::current().map_err(|e| e.to_string())? else {
                println!("gamesync: nothing outstanding");
                return Ok(());
            };
            let d = open()?;
            restore_lease(&d, &l)?;
            lease::release().map_err(|e| e.to_string())?;
            println!("gamesync: restored after {}", l.applied);
            Ok(())
        }
        // Called at startup. If a lease survived, the previous holder died
        // without restoring and the machine is still in its profile.
        ["gamesync", "recover"] => {
            let Some(l) = lease::current().map_err(|e| e.to_string())? else {
                println!("gamesync: no restore pending");
                return Ok(());
            };
            println!(
                "gamesync: restore pending from holder {:?} (applied {})",
                l.holder, l.applied
            );
            let d = open()?;
            restore_lease(&d, &l)?;
            lease::release().map_err(|e| e.to_string())?;
            println!("gamesync: recovered");
            Ok(())
        }
        ["gamesync", "status"] => {
            match lease::current().map_err(|e| e.to_string())? {
                None => println!("gamesync: no lease outstanding"),
                Some(l) => {
                    println!("holder    {}", l.holder);
                    println!("applied   {}", l.applied);
                    println!(
                        "baseline  {}",
                        if l.fallback_is_max() {
                            "none captured — recovery falls back to fans-max".to_string()
                        } else {
                            "captured".to_string()
                        }
                    );
        ...
        ["profile", "snapshot", name] => {
            let d = open()?;
            let p = Profile::snapshot(&d, (*name).to_string(), None).map_err(fmt)?;
            p.validate()?;
            profile::save(&p)?;
            println!("saved profile: {}", p.name);
            println!(
                "note: lighting was captured from firmware. Fan state was NOT:\n\
                 this firmware has no fan-mode getter, and the duty register is\n\
                 stale whenever the fans are in auto or maximum. The profile\n\
                 leaves fans untouched — add a `fans` key by hand if you want it\n\
                 to set them."
            );
            Ok(())
        }
...
/// Put the hardware back to a lease's baseline.
///
/// With no baseline the fallback is fans-at-maximum rather than the EC curve.
/// That is deliberate and specific to this hardware: auto permits 95 C and
/// 1446 MHz, and `acer-wmi` maps `pwm_enable = 0` to `ACER_WMID_FAN_MODE_TURBO`,
/// so the vendor driver treats "uncontrolled" as turbo too.
fn restore_lease(d: &Device, l: &lease::Lease) -> Result<(), String> {
    // Lighting first, where a baseline exists: it is the part the firmware can
    // actually attest to.
    if let Some(p) = &l.baseline {
        p.apply(d).map_err(fmt)?;
    }
    // Then fans, separately, because a baseline almost never carries a fan
    // policy — snapshot refuses to invent one. Applying the baseline alone
    // would restore the lighting and leave the fans wherever the lease put
    // them, which is the failure this whole mechanism exists to prevent.
    if l.fallback_is_max() {
        println!("fan state was not recoverable; forcing fans to maximum");
        d.fans_max().map_err(fmt)?;
    }
    Ok(())
}

/// A throttle counter, distinguishing "the kernel does not expose this" from
/// "the counter is zero". Conflating the two is how a machine with no counters
/// reads as a machine that never throttles.
fn print_counter(label: &str, value: Option<u64>) {
    match value {
        Some(v) => println!("{label}  {v}"),
        None => println!("{label}  not exposed by this kernel"),
    }
}
...
fn status(d: &Device) -> Result<(), String> {
    ...
    println!(
        "coolboost   {}",
        match d.coolboost() {
            Ok(true) => "on (getter-confirmed)",
            Ok(false) => "off (getter-confirmed)",
