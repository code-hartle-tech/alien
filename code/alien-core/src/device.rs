...
impl Device {
    ...
    pub fn boot_animation(&self) -> Result<Option<bool>> {
        self.transport.boot_animation().map_err(Error::from)
    }

    /// Set the boot animation only when its own getter reports support, with
    /// immediate readback and rollback on mismatch.
    pub fn set_boot_animation(&self, enabled: bool) -> Result<Option<bool>> {
        self.transport
