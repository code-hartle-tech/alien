...
impl Lighting {
    ...
    pub fn direction(&self, e: Effect) -> crate::rgb::Direction {
        let ltr = self.effects.get(e.name()).map(|m| m.left_to_right).unwrap_or(true);
        if ltr {
...
mod tests {
    ...
    fn a_file_predating_direction_still_loads() {
        ...
        assert_eq!(l.direction(Effect::Wave), crate::rgb::Direction::LeftToRight);
    }
    ...
    fn direction_is_remembered_per_effect() {
        ...
        assert_eq!(l.direction(Effect::Wave), crate::rgb::Direction::RightToLeft);
        assert_eq!(l.direction(Effect::Zoom), crate::rgb::Direction::LeftToRight);
    }
}
