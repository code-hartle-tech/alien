...
/// it lives on Linux.
///
/// **GPU** overclock does go through this function, with MHz offsets that
/// PredatorSense reads from `PredatorSense.ini [OC_GPU]`.
///
...
mod tests {
    ...
    fn single_fan_words_match_the_vendor_constants() {
        ...
        assert_eq!(
            FanBehaviour::Single { fan: Fan::Cpu, mode: FanMode::Auto }.to_word(),
            0x1_0001
        );
        assert_eq!(
            FanBehaviour::Single { fan: Fan::Cpu, mode: FanMode::Manual }.to_word(),
            0x3_0001
        );
        assert_eq!(
            FanBehaviour::Single { fan: Fan::Gpu, mode: FanMode::Auto }.to_word(),
            0x40_0008
        );
        assert_eq!(
            FanBehaviour::Single { fan: Fan::Gpu, mode: FanMode::Manual }.to_word(),
            0xC0_0008
        );
    }
