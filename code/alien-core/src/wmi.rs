...
/// # Measured effect
///
/// Switching from automatic to maximum is worth **+61.8 %** sustained CPU
/// throughput on the reference machine — 26 721 → 43 232 MIPS, measured across
/// a 4×20-minute ABBA run with the first half of each block discarded, against
/// same-condition drift of 9.0 % and 7.5 %.
///
/// The mechanism is the interesting part, and it is not temperature. Both
/// conditions pin the CPU at **92 °C**; the difference is the clock underneath,
/// **1446 MHz on the EC curve against 2406 MHz at maximum**. Board temperature
/// is what separates them — 83.4 °C versus 77.7 °C — because the chassis
/// saturates under the automatic curve, which never ramps past ~3529 RPM even
/// at 95 °C.
///
/// An earlier figure of +48 % from a same-session comparison is superseded by
/// this; it was measured over too short a load for heat soak to develop, which
/// on this chassis *is* the entire effect.
#[derive(Debug, Clone, Copy, PartialEq, Eq)]
