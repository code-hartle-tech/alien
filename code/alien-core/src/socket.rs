...
impl Transport for SocketClient {
    fn call_bytes(&self, function: u32, buf: &[u8]) -> Result<Vec<u8>, TransportError> {
        ...
        if n == 0 {
            // EOF, reported as an IO condition rather than an ACPI one.
            //
            // The distinction matters to a long-running client: `Io` means the
            // link died and the right move is to reconnect, while an
            // `AcpiFailure` means the firmware answered and said no. Filing
            // "the daemon exited" under the latter made a dead socket
            // indistinguishable from a refused call, so a GUI could only
            // report the last error forever instead of reconnecting.
