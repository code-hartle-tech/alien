...
const BOOT_ANIMATION_SELECTOR: [u8; 4] = 6u32.to_le_bytes();
...
pub trait Transport: Send + Sync {
    ...
    /// POST/boot animation state. `None` is a getter-confirmed absence.
    fn boot_animation(&self) -> Result<Option<bool>, TransportError> {
        Err(TransportError::UnsupportedEndpoint("boot animation"))
    }

    /// Set and getter-confirm the boot animation; `None` remains unsupported.
    fn set_boot_animation(&self, _enabled: bool) -> Result<Option<bool>, TransportError> {
        Err(TransportError::UnsupportedEndpoint("boot animation"))
    }
...
impl AcpiCall {
    ...
    // Boot animation: misc selector 6, recovered from PredatorSense's own
    // `Set_Post_AnimationSound` / `Get_Post_AnimationSound`
    // (`CommonFunction.cs:2507-2524`):
    //
    //   get: WMIGetGamingMiscSetting(6)
    //        success when (result & 0xff) == 0; state is (result >> 8) & 0xff
    //   set: WMISetGamingMiscSetting(6 | (state << 8))
    //        success when (result & 0xff) == 0
    //
    // `misc_word(6, state)` produces exactly that set payload.
    //
    // Acer declares this for the reference model: the PH315-53 plug's
    // `HW_Support.ini` lists `[AdvanceSetting] FUN1=Animation`, and the
    // checkbox bound to that name drives this selector.
    //
    // Self-gating in the same way LCD overdrive is. Any state byte outside 0/1
    // returns `None` — unsupported — rather than an error, so a machine whose
    // firmware does not implement the selector is never offered the control
    // and no write is ever attempted.
    fn boot_animation_unlocked(&self) -> Result<Option<bool>, TransportError> {
        let response = self.invoke_unlocked(
            GAMING_METHOD_EXACT,
            Function::GetMiscSetting as u32,
            &BOOT_ANIMATION_SELECTOR,
        )?;
        require_status_and_exact_len(&response, 8, "boot-animation getter")?;
        match response[1] {
            0 => Ok(Some(false)),
            1 => Ok(Some(true)),
            _ => Ok(None),
    ...
    fn write_boot_animation_unlocked(&self, enabled: bool) -> Result<(), TransportError> {
        let response = self.invoke_unlocked(
            GAMING_METHOD_EXACT,
            Function::SetMiscSetting as u32,
            &misc_word(6, enabled as u8).to_le_bytes(),
        )?;
        require_status_and_exact_len(&response, 4, "boot-animation setter")
    }
...
impl Transport for AcpiCall {
    ...
    fn boot_animation(&self) -> Result<Option<bool>, TransportError> {
        ...
        let _guard = self
            ...
            .lock()
            .unwrap_or_else(|poisoned| poisoned.into_inner());
        self.boot_animation_unlocked()
    }

    fn set_boot_animation(&self, enabled: bool) -> Result<Option<bool>, TransportError> {
        self.ensure_advanced_target()?;
        let _guard = self
            .call_lock
            .lock()
            .unwrap_or_else(|poisoned| poisoned.into_inner());
        // The getter gates the setter: no support, no write. This is what makes
        // it safe to ship a control recovered from decompilation but not yet
        // exercised on the reference machine.
        let Some(previous) = self.boot_animation_unlocked()? else {
            return Ok(None);
        };
        let write = self.write_boot_animation_unlocked(enabled);
        let readback = self.boot_animation_unlocked();
        if write.is_ok() && matches!(&readback, Ok(Some(current)) if *current == enabled) {
            return Ok(Some(enabled));
        }
        let rollback = match &readback {
            ...
            _ => match self.write_boot_animation_unlocked(previous) {
                Err(error) => format!("setter failed: {error}"),
                Ok(()) => match self.boot_animation_unlocked() {
                    Ok(Some(current)) if current == previous => "getter-confirmed".into(),
                    Ok(current) => format!("getter mismatched at {current:?}"),
                    Err(error) => format!("getter failed: {error}"),
                },
        ...
        Err(TransportError::AcpiFailure(format!(
            "boot-animation setter result {write:?}; readback {readback:?}; rollback to {previous}: {rollback}"
        )))
    }
