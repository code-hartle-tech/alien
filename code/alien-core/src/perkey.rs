...
//!   drives that.
//!
//! Alien implements and validates the four-zone path. This module is a
//! source-mapped, hardware-unverified ITE experiment: detection never reports
//! it as supported, packaged frontends receive no hidraw permission rule, and
//! the public CLI refuses writes until real hardware evidence closes that gap.
//!
...
impl Default for KeyFrame {
    fn default() -> Self {
        KeyFrame {
            keys: [[crate::rgb::Colour::OFF; COLS]; ROWS],
        }
    }
}
...
pub fn known_keys() -> Vec<&'static str> {
    vec![
        ...
        "3", "4", "5", "6", "7", "8", "9", "0", "tab", "q", "w", "e", "r", "t", "y", "u", "i", "o",
        "p", "caps", "a", "s", "d", "f", "g", "h", "j", "k", "l", "enter", "shift", "z", "x", "c",
        "v", "b", "n", "m", "ctrl", "alt", "space", "up", "down", "left", "right",
    ]
}
...
fn hidraw_node() -> Option<PathBuf> {
    ...
    for entry in fs::read_dir(&usb).ok()?.flatten() {
        ...
        if let Ok(children) = fs::read_dir(&hidraw) {
            if let Some(c) = children.flatten().next() {
...
mod tests {
    ...
    fn every_advertised_key_actually_maps() {
        ...
        for k in known_keys() {
            assert!(
                key_position(k).is_some(),
                "advertised key {k} has no position"
            );
        }
    }
    ...
    fn positions_are_inside_the_matrix() {
        for k in known_keys() {
            ...
            assert!(
                r < ROWS && c < COLS,
                "{k} maps outside the matrix at {r},{c}"
            );
        }
    }
