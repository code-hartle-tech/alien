...
mod tests {
    ...
    fn delta_reports_accumulated_events() {
        let before = ThrottleCounters { package_throttle: Some(100), ..Default::default() };
        let after = ThrottleCounters { package_throttle: Some(1_393_039), ..Default::default() };
    ...
    fn delta_saturates_when_a_counter_resets() {
        let before = ThrottleCounters { package_throttle: Some(500), ..Default::default() };
        let after = ThrottleCounters { package_throttle: Some(3), ..Default::default() };
        assert_eq!(after.delta(&before).package_throttle, Some(0));
    }

    #[test]
    fn zero_delta_is_not_throttling() {
        let a = ThrottleCounters { package_throttle: Some(42), ..Default::default() };
    ...
    fn idle_is_reported_but_is_not_a_limit() {
        ...
        assert!(g.gpu_idle);
        assert!(!g.is_limited(), "idle explains a low clock; it is not a cap");
        assert_eq!(g.summary(), "idle");
    }

    #[test]
    fn summary_reports_the_most_severe_reason_first() {
        let g = GpuLimiters::from_bits(
            GpuLimiters::SW_POWER_CAP | GpuLimiters::HW_THERMAL_SLOWDOWN,
        );
    ...
    fn every_documented_bit_decodes_to_its_own_field() {
        let cases = [
            ...
            (GpuLimiters::APPLICATIONS_CLOCKS, "applications-clocks setting"),
            ...
            (GpuLimiters::HW_THERMAL_SLOWDOWN, "hardware thermal slowdown"),
            (GpuLimiters::HW_POWER_BRAKE, "hardware power brake"),
            (GpuLimiters::DISPLAY_CLOCK_SETTING, "display clock setting"),
        ];
        for (bits, expected) in cases {
            assert_eq!(GpuLimiters::from_bits(bits).summary(), expected, "bits {bits:#x}");
        }
    }
}
