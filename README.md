...
<img src="assets/tech.hartle.Alien.gif" width="150" alt="Alien">
...
| CoolBoost | ✅ | ✅ |
| GPU Normal / Faster / Turbo | ✅ | ✅ <sub>PH315-53</sub> |
| Boot animation | ✅ | ✅ <sub>getter-gated</sub> |
...
<details>
<summary><b>What is missing, and how each one was settled</b></summary>

<br>

Each of these was decided against Acer's own shipped files — the decompiled
PredatorSense packages and the nineteen per-model support files inside them —
rather than by guessing from the outside.

**Keyboard backlight timeout** — *PredatorSense's own control does not work
here either.* This is not a gap in Alien. The checkbox reads a hotkey index
from `HKLM\SOFTWARE\OEM\PredatorSense\BK_Hotkey_Number`, whose helper
[defaults to `0` and writes that value back](docs/parity.md#keyboard-backlight-timeout)
when the key is absent. The reference model has `PerKey=0`, which routes it to
the WMI branch, so the call PredatorSense sends is byte-for-byte the
`0x00080001` Alien sends — and this firmware answers `0xe2`. No other index is
provisioned anywhere in the acquired artifacts. Alien declines to present the
control; PredatorSense presents one that does nothing, which is the same
"ships dead controls" behaviour the table above measures.

**Battery charge limit** — *not in the recovered PredatorSense versions at
all.* Not in the WMI method surface (IDs 9–35), not among the misc selectors,
and not declared by any of the nineteen model support files. The complete
declared feature set is Animation, AutoOverclocking, Backlight, Discrete_GPU,
LCD, Lightingeffect, Sound, Sticky_Key, Temperature and Windowskey1. Newer
Acer firmware and tooling do carry an 80% limiter; these versions do not, so
the row claimed a parity gap that never existed.

**The OSD overlay** — *not hardware control.* AppCenter manages Windows
applications, and a Linux OSD belongs at the compositor or session layer, not
inside a daemon that drives fans; building one here would be a worse version of
something the desktop already owns.

Boot animation used to sit in this list on the same reasoning. That was wrong:
it is misc selector 6, the reference model declares it as
`[AdvanceSetting] FUN1=Animation`, and it is now implemented and in the table.
